...
# Differential equations representing gravitational interactions
def f(s, t, m):
    N = len(m)
    ode = np.zeros((N, 6))
...
initial = np.array([
    [a1, 0, 0, v1, 0, 0],  # Jupiter-like planet
    [a2, 0, 0, v2, 0, 0]   # Saturn-like planet
])
m = [m1, m2]
t_arr, S_arr = Solver(5e8, 0, 10000, initial, f, m)

# Plot short-term simulation
plt.figure(figsize=(8, 8))
for i in range(len(m)):
    plt.plot(S_arr[:, i, 0] / 1e9, S_arr[:, i, 2] / 1e9, label=f'Body {i+1}')
...
plt.title("Two Planets Orbiting the Sun (RK4 Method)", fontsize=18)
plt.legend(fontsize=16, loc='upper right')
plt.axis('equal')
plt.grid()
plt.show()

# Second simulation (realistic Jupiter and Saturn, 30-year)
a1 = 5.204 * AU
m1 = 1.898e27
v1 = np.sqrt(G * M / a1)
...
# Plot realistic 30-year simulation
plt.figure(figsize=(8, 8))
...
plt.xlabel("x position (billion m)", fontsize=18)
plt.ylabel("y position (billion m)", fontsize=18)
plt.title("Jupiter and Saturn Orbiting the Sun (RK4, 30-year)", fontsize=18)
plt.legend(fontsize=16, loc='upper right')
plt.axis('equal')
plt.grid()
plt.show()
